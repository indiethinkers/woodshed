...
Woodshed is macOS-first, but the Cursor Cloud VM is **Linux (Ubuntu 24.04)**. It
runs the full native app under an already-running X server. Standard commands
live in `README.md` and `package.json`; the notes below are only the
Linux/cloud-specific caveats.

### Toolchain and environment (preinstalled on the VM snapshot)

- Bun lives at `~/.bun/bin/bun`. If a command reports `bun: not found`, prefix
  `PATH` with `~/.bun/bin` (bun is also added to `~/.bashrc` for login shells).
- Rust must be **stable ≥ 1.85** because pinned crates require `edition2024`.
  The VM's `rustup default` is set to `stable` (currently 1.97). The repo's
  `rust-version = "1.77"` in `Cargo.toml` is stale for building the locked deps.
- Linux Tauri system libraries (`libwebkit2gtk-4.1-dev`, `libgtk-3-dev`,
  `libayatana-appindicator3-dev`, `librsvg2-dev`, `libxdo-dev`, and
  `build-essential`) are already installed. The update script only refreshes JS
  deps via `bun install --frozen-lockfile`.

### Running the native app (`bun run tauri:dev`)

- Export `DISPLAY=:1` (an X server is always running there).
- Export `WEBKIT_DISABLE_COMPOSITING_MODE=1` and
...
- First compile of the Rust bin takes a few minutes; reuse the running
  `target/debug/woodshed` process instead of starting a second instance.
- Onboarding needs a writable vault path; `/home/ubuntu/woodshed` works. The
  "Create new vault" option scaffolds the native layout (with sample records) so
  the app is immediately usable without any Gmail/Calendar/Agent credentials.

### Verification caveats

- `bun run typecheck` fails until the TanStack Router route tree
  (`src/routeTree.gen.ts`, git-ignored and generated by the Vite router plugin)
  exists. Run `bun run build` (or start `bun run dev`/`tauri:dev`) once first to
  generate it, then typecheck passes.
- `cargo test --manifest-path src-tauri/Cargo.toml`: two macOS-only PDFKit tests
  in `commands::agent` fail on Linux (they error with "PDF attachments are
  supported by the macOS desktop app"). This is expected off macOS; the other
  ~616 tests pass.
